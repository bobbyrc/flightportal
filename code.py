...
from microcontroller import watchdog as dog
from watchdog import WatchDogMode

dog.timeout = 16  # watchdog timeout in seconds
...
try:
    ...
    print(
        "Secrets including geo-location are kept in secrets.py, please add them there!"
    )
    raise
...
# Colors and timings
ROW_ONE_COLOR = 0xEE82EE
ROW_TWO_COLOR = 0x4B0082
ROW_THREE_COLOR = 0xFFA500
...
matrixportal = MatrixPortal(headers=requestHeaders, esp=esp, rotation=0, debug=False)
...
label1 = adafruit_display_text.label.Label(FONT, color=ROW_ONE_COLOR, text="")
label1.x = 1
label1.y = 4

label2 = adafruit_display_text.label.Label(FONT, color=ROW_TWO_COLOR, text="")
label2.x = 1
label2.y = 15

label3 = adafruit_display_text.label.Label(FONT, color=ROW_THREE_COLOR, text="")
label3.x = 1
label3.y = 25
...
def initPlaneAnimation():
    ...
    planeGraphic = displayio.Group(x=matrixportal.display.width + 12, y=10)
    planeGraphic.append(planeTg)


# Scroll the plane bitmap right to left (same direction as scrolling text)
def plane_animation():
    matrixportal.display.show(planeGraphic)
...
def get_flight_details(fn):
    ...
    try:
        response = requests.get(url=FLIGHT_LONG_DETAILS_HEAD + fn, headers=requestHeaders)
...
def get_flights():
    ...
    try:
        ...
        response = requests.get(url=FLIGHT_SEARCH_URL, headers=requestHeaders).json()
...
# Actual doing of things - loop forever querying fr24, processing any results and waiting to query again
def main():
    checkConnection()
